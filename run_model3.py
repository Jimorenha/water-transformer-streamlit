...
n_steps_in = 104 # 输入时间维度
n_steps_out = 8 # 输出时间维度
...
def predict(model, dataframe, n_steps_in, n_steps_out, sc_x, sc_y):
    # sc_x, sc_y = train_get_scaler(96, 8)
...
# # 模型
# model = load_model('models/weather21_13(96_8).pth')
# # 天气
# weather_path = 'datasets/天气.xls'
# weather = manageData.get_2day_weather(weather_path)
# # 需水量
# uploaded_file = 'datasets/报表查询.xls'
# dataset_2day = manageData.get_2day_df(uploaded_file)
# actual_date = pd.date_range('2021/08/09 12:15', '2021/08/11 12:00', freq='15MIN')
# dataset_2day.insert(0, 'date', actual_date)
# # 添加month/hour/holiday列
# data_2day = manageData.get_2day_feature(actual_date, dataset_2day)
# merge_2day = pd.merge(data_2day, weather, on=['month', 'hour', 'day'])
# result_2day = merge_2day[
#     ['date_x', 'month', 'hour', 'holiday', 'temp', 'humidity', 'rainfall', 'YLEJ', 'YLSJ_LS', 'YLSJ_CY', 'YLSJ',
#      'YLLY_SJ', 'YLWJ', 'LTEJ_LJYZ', 'LTEJ_SSQ', 'LTSJ',
#      'JBEJ', 'JCSJ', 'YBEJ', 'YBSJ']]
#
# pre = predict(model, result_2day, n_steps_in, n_steps_out)
